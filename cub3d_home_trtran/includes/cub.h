...
typedef struct s_cub
{
	...
	int			w_map;
	int			h_map;
	double		dir_vec_x; // coordinate of unit vector
	double		dir_vec_y; // coordinate of unit vector
	double		s_dist_x; // side distance from X
	double		s_dist_y; // side distance from Y
	double		d_dist_x; // distance between two parallel X lines
	double		d_dist_y; // distance between two parallel Y lines
	int			pl_map_x; //coordinate of player in map
	int			pl_map_y; //coordinate of player in map
	double		pl_board_x; //coordinate of player board
	double		pl_board_y; //coordinate of player board
	char		compass_x; //crossing direction with map for x (side of the horizon)
	char		compass_y; //crossing direction with map for y (side of the horizon)
	char		pic; // xpm picture name to draw
	int			w_or_d; // 0 - space, 1/2 - wall, 3 - closed door, 4 - halfopen door
	double		fov; //field of view
	double		cntr_rad; //central radian
	double		cross_coor_x; // crossing X coordinate of ray with wall
	double		cross_coor_y; // crossing y coordinate of ray with wall
	double		ray_length; // length of current ray
	double		ray_projection; // projection of current ray on camera plane
	int			i;
}			t_cub;

struct	s_vertical // crossing point coordinates with X line
{
	double		x;
	double		y;
	double		ray_length;
...
struct	s_horizon // crossing point coordinates with Y line
{
	double		x;
	double		y;
	double		ray_length;
...
char		**ft_get_xpm_paths_arr(char *map_path, char *buff, int cnt);
void		ft_xpm_path_check(char *map_path, t_cub *c);
int			ft_comma_count(char *str);
void		ft_check_comma_count(char *map_path, char *buff, int cnt);
int			ft_allowed_chars(char *str);
void		ft_check_allowed_chars(char *map_path, char *buff, int cnt);
int			ft_check_long_long_limits(char *str);
char		**ft_get_color_subint_arr(char *map_path, char *buff, int cnt);
void		ft_set_colors(t_cub *c, char **colors);
...
char		**ft_remove_first_str_from_arr(char **arr);
char		**ft_read_map(char *map_path, char *buff);
char		**ft_empty_strs_in_arr(char **arr);
char		**ft_get_map(char *map_path, t_cub *c);
size_t		ft_max_strlen_in_arr(char **arr);
void		ft_align_arr(char **arr);
void		ft_print_line(char c, int len);
void		ft_invalid_neighbors(char **m, int y, int x);
...
int			ft_draw_red_triangle(t_cub *c);
